...
(****************************************************************************************)
type automaton = {
	a_alphabet: symbol array;
	a_states: state array;
	a_rules: rule list;
}
and symbol = {
	f_index: int;
...
let sn a = Array.length a.a_states
let rn a = List.length a.a_rules
let an a = Array.length a.a_alphabet
...
*)
(****************************************************************************************)
(* Some additional functions for simulation experiments*)
(****************************************************************************************)


(*I want exactly one leaf rule for each state, all the leaf symbols the same *)
let unify_leaf_rules aut =
	let withou_leaf_rules = List.filter (fun {r_lhs = lhs} -> lhs <> [||]) aut.a_rules in
	let leaf_symbol = List.find (fun {f_rank = ar} -> ar = 0) (Array.to_list aut.a_alphabet) in
	let new_rules = 
		Array.fold_left (fun rules q  -> 
			{	r_symbol = leaf_symbol.f_index;
				r_lhs = [||];
				r_rhs = q.s_index;
			}::rules
		) withou_leaf_rules aut.a_states 
	in
...
let leaf_rules_into_initial_state a = 
	let leaf_rules,non_leaf_rules = 
		List.partition (fun r -> a.a_alphabet.(r.r_symbol).f_rank = 0) a.a_rules 
	in
	let leaf_rule_groups = sort_and_split (rule_comparator By_symbol) leaf_rules in
	match leaf_rule_groups with
	[] -> a
	| _ ->
			let s_n = sn a in
			let f_n = Array.length a.a_alphabet in
			let init_state = {s_name = "q_init";s_index = s_n ;s_final = false} in
			let new_states = Array.append (Array.copy a.a_states) [|init_state|] in
			let new_symbol0 = {f_index = f_n;f_name = "init0"; f_rank = 0} in
			let new_symbol1 = {f_index = f_n+1;f_name = "init1"; f_rank = 1} in
			let new_rules0 = {r_lhs = [||];r_symbol = f_n; r_rhs = s_n}
								::{r_lhs = [|s_n|];r_symbol = f_n + 1; r_rhs = s_n}
								::non_leaf_rules
			in
			let new_alphabet0 = Array.append (Array.copy a.a_alphabet) [|new_symbol0;new_symbol1|] in

			let process_group (alphabet,rules) group = 
				let new_f_n = Array.length alphabet in
				let f_name = a.a_alphabet.((List.hd group).r_symbol).f_name in
				let new_symbol = {f_index = new_f_n;f_name = "init_"^f_name; f_rank = 1} in
				let new_alphabet = Array.append alphabet [|new_symbol|] in
				let additional_rules = 
					List.map (fun {r_rhs = q} -> 
						{r_lhs = [|s_n|];r_symbol = new_f_n; r_rhs = q}
					) group
				in
				new_alphabet,additional_rules@rules
			in

			let (new_alphabet,new_rules) = 
				List.fold_left (fun (alphabet,rules) group -> 
					process_group (alphabet,rules) group
				)  (new_alphabet0,new_rules0) leaf_rule_groups
			in

			{
				a_alphabet = new_alphabet;
				a_states = new_states;
				a_rules = new_rules;
			}
;;

let final_states_into_one a = 
	let final_states = final_states a in 
	let states =
		Array.map (fun q -> {s_name = q.s_name;s_index = q.s_index;s_final = false}) a.a_states
	in
	let s_n = sn a in
	let f_n = Array.length a.a_alphabet in
	let final_state = {s_name = "q_final";s_index = s_n ;s_final = true} in
	let new_states = Array.append states [|final_state|] in
	let new_symbol0 = {f_index = f_n;f_name = "final0"; f_rank = 1} in
	let new_symbol1 = {f_index = f_n+1;f_name = "final1"; f_rank = 1} in
	let new_rules0 = {r_lhs = [|s_n|];r_symbol = f_n + 1; r_rhs = s_n}::a.a_rules in
	let new_alphabet = Array.append (Array.copy a.a_alphabet) [|new_symbol0;new_symbol1|] in
	let make_final_rule q = {r_lhs = [|q.s_index|];r_symbol = f_n; r_rhs = s_n} in
	let new_rules = (List.map make_final_rule final_states)@new_rules0 in
...
let everybodi_final {a_states = states; a_rules = rules; a_alphabet = alphabet} = 
	let new_states = 
		Array.map (fun {s_index=i; s_name=name} -> 
			{s_index = i; s_name = name; s_final = true })
		states	
	in
	{a_states = new_states;a_rules = rules; a_alphabet = alphabet}
...
(*I suppose that there is exactly one symbol of rank 0*)
let reverse_word_automaton {a_states = states; a_rules = rules; a_alphabet = alphabet} =
	let init_symbol = (List.find (fun f -> f.f_rank = 0) (Array.to_list alphabet)).f_index in
	let init_states = List.fold_left 
		(fun set {r_symbol = f; r_rhs = q} -> if f = init_symbol then Int_set.add q set else set) 
	Int_set.empty rules in
	let without_leaf_rules = List.filter (fun {r_lhs = lhs} -> lhs <> [||]) rules in
	let reverse_rule {r_lhs = [|q|]; r_symbol = a; r_rhs = r} = {r_lhs = [|r|]; r_symbol = a; r_rhs = q} in
	let reversed = List.map reverse_rule without_leaf_rules in
	let final_states = List.map (fun q -> q.s_index) 
		(List.filter (fun q -> q.s_final) (Array.to_list states)) 
	in
	let new_init_rules = List.map (fun q -> {r_lhs = [||]; r_symbol = init_symbol; r_rhs = q}) final_states in
(*	List.iter (fun q -> new_states.(q).s_final <- false) final_states;*)
(*	Int_set.iter (fun q -> new_states.(q).s_final <- true) init_states;*)
	let new_states = Array.map (fun {s_index = i; s_name = n} -> 
		{s_index = i; s_name = n; s_final = (Int_set.mem i init_states) })
		states
	in
...
(*	let res = *)
	{
		a_states = new_states;
		a_rules = reversed@new_init_rules;
		a_alphabet = alphabet
	} 
(*	in*)
(*	print {a_states = states; a_rules = rules; a_alphabet = alphabet};*)
(*	print res;*)
(*	res*)
	
let nondeterminism_no {a_rules = rules} = 
	let vectors = sort_and_split (fun r1 r2 -> compare (r1.r_symbol,r1.r_lhs) (r2.r_symbol,r2.r_lhs)) rules in
	(List.length rules) - (List.length vectors)
